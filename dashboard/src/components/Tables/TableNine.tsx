...
const LoremModal: React.FC<{ modal: typeof BasicModal }> = ({ modal }) => {
  const { loader, successMessage, errorMessage, menus, menu } = useSelector(
    (state) => state?.food
  );
  ...
  useEffect(() => {
    ...
    setSelectedStatus(status.filter((x) => x.value === menu?.status));
  }, [menu]);
  ...
  useEffect(() => {
    ...
    if (successMessage) {
      ...
      setTimeout(() => dispatch(menus_get()), 1000);
    }
  ...
  return (
    ...
    >
      <modal.Head>{menu ? "Menu Update 🙋‍♀️" : "Menu Entry 🙋‍♀️"}</modal.Head>
      <modal.Body>
...
const TableNine = () => {
  const [params, setParams] = useSearchParams();
  const { menus } = useSelector((state) => state?.food);
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(menus_get());
  }, [dispatch]);
          ...
          <div className="flex items-center justify-between gap-8 mb-8">
            <div>
              ...
              <p className="block mt-1 font-sans text-base antialiased font-normal leading-relaxed text-gray-700 dark:text-white">
                See information about all menus
              </p>
            </div>
