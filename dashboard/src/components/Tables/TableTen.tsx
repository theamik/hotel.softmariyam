...
const LoremModal: React.FC<{ modal: typeof BasicModal }> = ({ modal }) => {
  const { loader, successMessage, errorMessage, user } = useSelector(
    (state) => state?.auth
  );
  ...
  const roles = [
    ...
    { value: "res_admin", label: "Restaurant Admin" },
    { value: "hot_res_admin", label: "Hotel Restaurant Admin" },
  ];

  useEffect(() => {
    setName(user?.name);
    setEmail(user?.email);
    setPassword(user?.etPassword);
    setPosition(user?.position);
    setMobile(user?.mobile);
    setSelectedStatus(status.filter((x) => x.value === user?.status));
    setSelectedRole(roles.filter((x) => x.value === user?.role));
  }, [user]);
  ...
  useEffect(() => {
    ...
    if (successMessage) {
      ...
      setTimeout(() => dispatch(get_all_users()), 1000);
    }
