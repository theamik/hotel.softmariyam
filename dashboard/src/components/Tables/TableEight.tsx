...
import { messageClear } from "../../store/Reducers/roomReducer";

const LoremModal: React.FC<{ modal: typeof BasicModal }> = ({ modal }) => {
  const { loader, successMessage, errorMessage, categories, room } =
  ...
  const finalArr = categories.map((el) => ({
    value: el["_id"],
    label: el["name"],
  }));
  ...
  useEffect(() => {
    ...
    setSelectedStatus(status.filter((x) => x.value === room?.status));
    setTimeout(
      () =>
        setSelectedCategory(
          finalArr?.filter((x) => x.value === room?.categoryId)
        ),
      1000
    );
  ...
  useEffect(() => {
    ...
    if (successMessage) {
      ...
      setTimeout(() => dispatch(rooms_get()), 1000);
    }
  ...
  return (
    ...
    >
      <modal.Head>{room ? "Room Update 🙋‍♀️" : "Room Entry 🙋‍♀️"}</modal.Head>
      <modal.Body>
...
const TableEight = () => {
  const [params, setParams] = useSearchParams();
  const { rooms, totalRoom } = useSelector((state) => state?.room);
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(rooms_get());
    dispatch(categories_get());
        ...
        <div className="relative mx-4 mt-4 overflow-hidden text-gray-700 dark:text-white bg-white dark:bg-boxdark rounded-none bg-clip-border">
          ...
          <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
            <div className="block w-full overflow-hidden md:w-max">
              <p className="text-xl">Total Room : {totalRoom}</p>
            </div>
              ...
              {rooms &&
                rooms?.map((d, i) => (
                  <tr>
                    ...
                    <td className="p-4 border-b border-blue-gray-50">
                      <p className="block font-sans text-sm antialiased font-normal leading-normal text-blue-gray-900">
                        {d.categoryId?.occupancy}
                      </p>
                    </td>
                    <td className="p-4 border-b border-blue-gray-50">
                      <p className="block font-sans text-sm antialiased font-normal leading-normal text-blue-gray-900">
                        {d.categoryId?.rackRate}
                      </p>
                    </td>
                    <td className="p-4 border-b border-blue-gray-50">
                      <div className="flex flex-col">
                        <p className="block font-sans text-sm antialiased font-normal leading-normal text-blue-gray-900">
                          {d.categoryId?.discountRate}
                        </p>
                      </div>
                    </td>
                    <td className="p-4 border-b border-blue-gray-50">
