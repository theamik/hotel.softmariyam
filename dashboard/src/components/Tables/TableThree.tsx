...
const LoremModal: React.FC<LoremModalProps> = ({ modal }) => {
  ...
  const updateHandler = (e: React.FormEvent) => {
    ...
    // The `useEffect` in TableThree will handle the re-fetch via successMessage.
    params.delete("modal");
    setParams(params);
  };
  ...
  useEffect(() => {
    ...
    if (successMessage) {
      ...
      // This will be handled by the TableThree component's useEffect, which listens to successMessage
    }
