...
const LoremModal: React.FC<{ modal: typeof BasicModal }> = ({ modal }) => {
  ...
  useEffect(() => {
    ...
    setOccupancy(category?.occupancy);
    setRackRate(category?.rackRate);
    setDiscountRate(category?.discountRate);
    setDescription(category?.description);
    setSelectedStatus(status.filter((x) => x.value === category?.status));
  }, [category]);
  ...
  useEffect(() => {
    ...
    if (successMessage) {
      ...
      setTimeout(() => dispatch(categories_get()), 1000);
    }
  ...
  return (
    ...
    >
      <modal.Head>
        {category ? "Category Update 🙋‍♀️" : "Category Entry 🙋‍♀️"}
      </modal.Head>
      <modal.Body>
